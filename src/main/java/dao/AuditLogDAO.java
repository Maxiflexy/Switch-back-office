...
public class AuditLogDAO {
    ...
    final static Logger LOG = LogManager.getLogger(AuditLogDAO.class);
...
//    }
    ...
    public List<BaseBean> getAuditLogs(String startDate, String endDate) throws SQLException{
        List<BaseBean> auditLogs = new ArrayList<>();
        int k = 0;
        String query = "SELECT * FROM "
                .concat(AUDIT_LOG)
                .concat(" WHERE operation_date between TO_DATE(?, 'YYYY-MM-DD\"T\"HH24:MI:SS') AND TO_DATE(?, 'YYYY-MM-DD\"T\"HH24:MI:SS')");

        LOG.info("audit Query: {}", query);
        try (Connection conn = ConnectionUtil.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(query);
             ) {
