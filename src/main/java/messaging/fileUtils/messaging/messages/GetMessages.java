package messaging.fileUtils.messaging.messages;

import messaging.fileUtils.persistence.MessageDbHelper;
...
public class GetMessages implements RequestExecutor {
    ...
    public String execute(String request, String currentUser, String actionId) {
        ...
        boolean response = MessageDbHelper.fetchMessages(requestBean);
    ...
    private String createReply(BaseBean requestBean, boolean error) {
        ...
        String message = "Successfully fetched messages";
        JsonObject messages = JsonUtil.toJsonObject(requestBean.getString("messages"));
        jsonResp = jObjBuil.add("status", ResponseUtil.SUCCESS)
                .add("message", message)
                .add("data", messages.getJsonArray("data"))
                .build();
