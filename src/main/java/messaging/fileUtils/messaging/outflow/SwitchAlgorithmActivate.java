package messaging.fileUtils.messaging.outflow;
...
public class SwitchAlgorithmActivate extends RequestValidator implements RequestExecutor {

    final static Logger LOG = LogManager.getLogger(SwitchAlgorithmActivate.class);
    ...
    public String execute(String request, String currentUser, String actionId) {
        ...
        if (validateRequestBody(requestBean, request) && !"01".equals(requestBean.getString("validationcode"))) {
            requestBean.setString("action", requestBean.getString("status").equalsIgnoreCase("activate") ? OutflowSwitchAction.ACTIVATE.getName() : OutflowSwitchAction.DEACTIVATE.getName());
