...
import messaging.fileUtils.persistence.SwitchDbHelper;
...
public class UpdateSwitch extends RequestValidator implements RequestExecutor {

    final static Logger LOG = LogManager.getLogger(UpdateSwitch.class);

    @Override
    public String execute(String request, String currentUser, String actionId) {
        BaseBean requestBean = new BaseBean();
        requestBean.setString("user", currentUser);
        requestBean.setString("mc-action", AppConstants.AppActions.UPDATE);
    ...
    public boolean validateRequest(BaseBean requestBean, String request) {
        ...
        try {
            ...
            validateOptionalParameter(jsonRequest, requestBean, "switch-name", true);
            validateOptionalParameter(jsonRequest, requestBean, "switch-code", true);
