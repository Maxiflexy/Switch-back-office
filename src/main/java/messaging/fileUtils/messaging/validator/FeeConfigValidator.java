...
public class FeeConfigValidator extends RowValidator implements FileValidator {
    static final Logger LOG = LogManager.getLogger(FeeConfigValidator.class);

    private final Map<String, Object> fileData = new HashMap<>();
    private final String[] headerColumnCount = new String[]{"service_id","fee_amt","cr_acc_derivation_flg", "cr_acct_prefix", "cr_acct_suffix","cr_acct_num", "channel_id", "fix_amt", "add_to_amt", "status"};
    private final String[] inputColumns = new String[]{"service_id","fee_amt","cr_acc_derivation_flg", "cr_acct_prefix", "cr_acct_suffix","cr_acct_num", "channel_id", "fix_amt", "add_to_amt", "status"};

    private Reader reader = null;

    public FeeConfigValidator(Reader reader) {
        this.reader = reader;

    }

    public FeeConfigValidator() {
    }

    @Override
    public boolean validateFileHeader(InputStream request, BaseBean validationBean) {
        java.io.Reader reader = new InputStreamReader(request);
    ...
    private static void extractFileDetails(BaseBean requestBean, List<BaseBean> tables, InputStream inputStream) throws IOException, CsvValidationException {
        ...
        FeeConfigValidator validator = new FeeConfigValidator();
        try (CSVReader csvReader = new CSVReaderBuilder(reader)
                .build()) {
            String[] nextLine = csvReader.readNext();
            while ((nextLine = csvReader.readNext()) != null) {
                ...
                row.setString("fee_amt", nextLine[1]);
                row.setString("cr_acc_derivation_flg", nextLine[2]);
                row.setString("cr_acct_prefix", nextLine[3]);
                row.setString("cr_acct_suffix", nextLine[4]);
                row.setString("cr_acct_num", nextLine[5]);
                row.setString("channel_id", nextLine[6]);
                row.setString("fix_amt", nextLine[7]);
                row.setString("add_to_amt", nextLine[8]);
                row.setString("document-id", id);
                row.setString("status", nextLine[9]);
    ...
    protected   boolean validateRow(BaseBean row, BaseBean requestBean) {
        ...
        if (!DBHelper.fetchServiceId(new BaseBean()).contains(row.getString("service_id"))) {
            requestBean.setString("message","Invalid service ID");
            throw new CustomException(requestBean);
        }
        if (!DBHelper.fetchUserNameIds(new BaseBean()).contains(row.getString("channel_id"))) {
            requestBean.setString("message","Invalid channel ID");
            throw new CustomException(requestBean);
        }

        if (row.getString("fee_amt").isEmpty()) {
            requestBean.setString("message", "Invalid fee amount");
            throw new CustomException(requestBean);
        }

        if (row.getString("cr_acc_derivation_flg").isEmpty() && !row.getString("cr_acc_derivation_flg").equals("true") && !row.getString("cr_acc_derivation_flg").equals("false")) {
            requestBean.setString("message", "Invalid account derivation flag");
            throw new CustomException(requestBean);
        } else if (row.getString("cr_acc_derivation_flg").equals("true")) {
            if (row.getString("cr_acc_prefix").isEmpty() || row.getString("cr_acc_suffix").isEmpty()) {
                requestBean.setString("message", "Invalid account derivation flag");
                throw new CustomException(requestBean);
            }
        } else if (row.getString("cr_acc_derivation_flg").equals("false")) {
            if (row.getString("cr_acct_num").isEmpty()) {
