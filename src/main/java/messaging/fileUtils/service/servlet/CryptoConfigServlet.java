...
import static constants.RegAppConstants.TB_CRYPTO_CONFIG;
import static util.JwtUtil.getUsername;

public class CryptoConfigServlet extends BaseServlet {

    private final CryptoConfigDAO cryptoConfigDAO;
    private final AuditLogDAO auditLogDAO;
    private final CustomDAO customDAO;

    public CryptoConfigServlet() {
        this.cryptoConfigDAO = new CryptoConfigDAO();
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (uri.equals("/switch/crypto_config/create")) {
    ...
    protected void doPut(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (id != null) {
            try {
                if (uri.startsWith("/switch/crypto_config/update")) {
                    updateApiReg(req, resp, id);
                } else if (uri.startsWith("/switch/crypto_config/approval")) {
                    handleApproval(req, resp, id);
                } else if (uri.startsWith("/switch/crypto_config/enable_disable")) {
    ...
    protected void doDelete(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (id != null) {
            try {
                cryptoConfigDAO.deleteCryptoConfig(Integer.parseInt(id));
            ...
            resp.sendError(HttpServletResponse.SC_BAD_REQUEST, "Missing app_code or switch_code parameter");
        }
    }
    ...
    private BaseBean createBaseBean(JsonObject jsonObject) {
        ...
        cryptoConfig.put("app_code", JsonUtil.getJsonObjValue(jsonObject, "app_code"));
        cryptoConfig.put("switch_code", JsonUtil.getJsonObjValue(jsonObject, "switch_code"));
        cryptoConfig.put("its_private_keyfile", JsonUtil.getJsonObjValue(jsonObject, "its_private_keyfile"));
        cryptoConfig.put("its_public_keyfile", JsonUtil.getJsonObjValue(jsonObject, "its_public_keyfile"));
        cryptoConfig.put("switch_public_keyfile", JsonUtil.getJsonObjValue(jsonObject, "switch_public_keyfile"));
    ...
    private void processValidRequest(HttpServletRequest req, HttpServletResponse resp, String body, BaseBean bean) throws SQLException, IOException {
        ...
        boolean status = cryptoConfigDAO.insertCryptoConfig(bean);
    ...
    private void logAudit(String body, BaseBean bean) throws SQLException {
        ...
        auditBean.put("table_name", TB_CRYPTO_CONFIG);
    ...
    private void handleRequestWithId(String id, String uri, HttpServletResponse resp) throws IOException, SQLException {
        List<BaseBean> beans = cryptoConfigDAO.getAllCryptoConfigs();

        if (uri.startsWith("/switch/crypto_config/get_pending_id")) {
            writeResponse(resp, getById(beans, id, "pending"));
        } else if (uri.startsWith("/switch/crypto_config/get_approved_by_id")) {
    ...
    private void handleRequestWithoutId(String uri, HttpServletResponse resp) throws IOException, SQLException {
        if (uri.equals("/switch/crypto_config/get_approved")) {
            List<BaseBean> records = cryptoConfigDAO.getCryptoConfigByApproval("approved");
            writeResponse(resp, JsonUtil.convertBaseBeanListToJsonString(records));
        } else if (uri.equals("/switch/crypto_config/get_pending")) {
            List<BaseBean> records = cryptoConfigDAO.getCryptoConfigByApproval("pending");
            writeResponse(resp, JsonUtil.convertBaseBeanListToJsonString(records));
        }
    }
    ...
    private void handleApproval(HttpServletRequest req, HttpServletResponse resp, String id) throws IOException, SQLException {
        ...
        if (ValidateResponse.isValidResponse(jsonObject, bean)) {
            ...
            customDAO.approveOrReject(bean, ITS_CRYPTO_CONFIG);
    ...
    private void handleEnableDisable(HttpServletRequest req, HttpServletResponse resp, String id) throws SQLException, IOException {
        ...
        if (ValidateResponse.isValidResponse(jsonObject, bean)) {
            bean.put("id", id);
            customDAO.disableRow(bean, ITS_CRYPTO_CONFIG);
    ...
    private void updateApiReg(HttpServletRequest req, HttpServletResponse resp, String id) throws SQLException, IOException {
        ...
        bean.put("its_private_keyfile", JsonUtil.getJsonObjValue(jsonObject, "its_private_keyfile"));
        bean.put("its_public_keyfile", JsonUtil.getJsonObjValue(jsonObject, "its_public_keyfile"));
        bean.put("switch_public_keyfile", JsonUtil.getJsonObjValue(jsonObject, "switch_public_keyfile"));
        ...
        if (ValidateResponse.isValidResponse(jsonObject, bean)) {
            ...
            String oldValue = JsonUtil.convertBaseBeanToStr(cryptoConfigDAO.getCryptoConfigById(Integer.parseInt(id)));
            boolean success = cryptoConfigDAO.updateCryptoConfig(bean);
            String newValue = JsonUtil.convertBaseBeanToStr(cryptoConfigDAO.getCryptoConfigById(Integer.parseInt(id)));
