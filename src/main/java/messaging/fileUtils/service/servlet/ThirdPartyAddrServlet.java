...
public class ThirdPartyAddrServlet extends BaseServlet {
    ...
    private ThirdPartyAddrDAO thirdPartyAddrDAO;
    private AuditLogDAO auditLogDAO;
    private CustomDAO customDAO;

    public ThirdPartyAddrServlet() {
        this.thirdPartyAddrDAO = new ThirdPartyAddrDAO();
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (uri.equals("/switch/thirdPartyAddr/create")) {
    ...
    protected void doPut(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (id != null) {
            try {
                if (uri.startsWith("/switch/thirdPartyAddr/update")) {
                    updateApiReg(req, resp, id);
                } else if (uri.startsWith("/switch/thirdPartyAddr/approval")) {
                    handleApproval(req, resp, id);
                } else if (uri.startsWith("/switch/thirdPartyAddr/enable_disable")) {
    ...
    protected void doDelete(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (id != null) {
            try {
                thirdPartyAddrDAO.deleteThirdPartyAddr(Integer.parseInt(id));
    ...
    private BaseBean createBaseBean(JsonObject jsonObject) {
        ...
        bean.put("ip_addr", JsonUtil.getJsonObjValue(jsonObject, "ip_addr"));
    ...
    private void processValidRequest(HttpServletRequest req, HttpServletResponse resp, String body, BaseBean bean) throws SQLException, IOException {
        ...
        if (RequestValidator.validateIPAddress(bean.getString("ip_addr"))) {
            status = thirdPartyAddrDAO.insertThirdPartyAddr(bean);
        } else {
            sendErrorResponse(resp, "Invalid IP address", HttpServletResponse.SC_BAD_REQUEST);
            return;
        }
        if (!status) {
    ...
    private void logAudit(String body, BaseBean bean) throws SQLException {
        ...
        auditBean.put("table_name", TB_3RD_PARTY_ADDR);
    ...
    private void handleRequestWithId(String id, String uri, HttpServletResponse resp) throws IOException, SQLException {
        List<BaseBean> beans = thirdPartyAddrDAO.getAllThirdPartyAddrs();

        if (uri.startsWith("/switch/thirdPartyAddr/get_pending_id")) {
            writeResponse(resp, getById(beans, id, "pending"));
        } else if (uri.startsWith("/switch/thirdPartyAddr/get_approved_by_id")) {
    ...
    private void handleRequestWithoutId(String uri, HttpServletResponse resp) throws IOException, SQLException {
        if (uri.equals("/switch/thirdPartyAddr/get_approved")) {
            List<BaseBean> records = thirdPartyAddrDAO.getThirdPartyAddrByApproval("approved");
            writeResponse(resp, JsonUtil.convertBaseBeanListToJsonString(records));
        } else if (uri.equals("/switch/thirdPartyAddr/get_pending")) {
            List<BaseBean> records = thirdPartyAddrDAO.getThirdPartyAddrByApproval("pending");
            writeResponse(resp, JsonUtil.convertBaseBeanListToJsonString(records));
        }
    }
    ...
    private void handleApproval(HttpServletRequest req, HttpServletResponse resp, String id) throws IOException, SQLException {
        ...
        if (ValidateResponse.isValidResponse(jsonObject, bean)) {
            ...
            customDAO.approveOrReject(bean, ITS_3RD_PARTY_ADDR);
    ...
    private void handleEnableDisable(HttpServletRequest req, HttpServletResponse resp, String id) throws SQLException, IOException {
        ...
        if (ValidateResponse.isValidResponse(jsonObject, bean)) {
            bean.put("id", id);
            customDAO.disableRow(bean, ITS_3RD_PARTY_ADDR);
    ...
    private void updateApiReg(HttpServletRequest req, HttpServletResponse resp, String id) throws SQLException, IOException {
        ...
        if (ValidateResponse.isValidResponse(jsonObject, bean)) {
            ...
            String oldValue = JsonUtil.convertBaseBeanToStr(thirdPartyAddrDAO.getThirdPartyAddrById(Integer.parseInt(id)));
            boolean success = thirdPartyAddrDAO.updateThirdPartyAddr(bean);
            String newValue = JsonUtil.convertBaseBeanToStr(thirdPartyAddrDAO.getThirdPartyAddrById(Integer.parseInt(id)));
