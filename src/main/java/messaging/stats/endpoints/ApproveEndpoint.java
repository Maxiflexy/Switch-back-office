package messaging.stats.endpoints;
...
public class ApproveEndpoint extends RequestValidator implements RequestExecutor {

    final static Logger LOG = LogManager.getLogger(ApproveEndpoint.class);
    ...
    public String execute(String request, String currentUser, String actionId) {
        ...
        if (validateRequest(requestBean, request) && !"01".equals(requestBean.getString("validationcode"))) {
            result = PerformanceDbHelper.approveEndpointRequest(requestBean);
            LOG.info("Endpoint request approved/cancelled successfully: {}", result);
        }
    ...
    private String createReply(BaseBean requestBean, boolean error) {
        ...
        String message = "Endpoint request ".concat(Boolean.parseBoolean(requestBean.getString("approval-status")) ? "approved" : "cancelled");
