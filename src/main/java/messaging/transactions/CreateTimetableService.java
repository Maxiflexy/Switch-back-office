...
public class CreateTimetableService implements RequestExecutor {
    ...
    public String execute(String request, String currentUser, String actionId) {
        ...
        try {
            ...
            // Check if this is an update request (PUT)
            if (jsonRequest != null && "update".equals(JsonUtil.getJsonObjValue2(jsonRequest, "action"))) {
                return handleUpdateRequest(jsonRequest, requestBean, currentUser, actionId);
            }
...
//            }
    ...
    /**
     * Handle PUT request to update timetable data
     */
    private String handleUpdateRequest(JsonObject jsonRequest, BaseBean requestBean, String currentUser, String actionId) {
        try {
            // Extract parameters from request
            if (jsonRequest != null) {
                requestBean.setString("channel_code", JsonUtil.getJsonObjValue2(jsonRequest, "channelCode"));
...
//            }
            ...
            for (int i = 0; i < switchDetailsArray.size(); i++) {
                ...
                // Validate ID
                String idStr = JsonUtil.getJsonObjValue2(switchDetail, "id");
                if (idStr.trim().isEmpty()) {
                    return createErrorResponse("400", "id is required for entry " + (i + 1));
                }
                ...
                try {
                    ...
                    if (id < 1) {
                        return createErrorResponse("400", "id must be greater than 0 for entry " + (i + 1));
                    }
                } catch (NumberFormatException e) {
                    return createErrorResponse("400", "id must be a valid number for entry " + (i + 1));
                }

                // Check for duplicate IDs in this request
                if (usedIds.contains(id)) {
                    return createErrorResponse("400", "Duplicate id " + id + " found in request. Each record can only be updated once.");
                }
                usedIds.add(id);

                // Validate switchCode
                String switchCode = JsonUtil.getJsonObjValue2(switchDetail, "switchCode");
                if (switchCode.trim().isEmpty()) {
                    return createErrorResponse("400", "switchCode is required for entry " + (i + 1));
                }
                switchCode = switchCode.trim().toUpperCase();
                if (switchCode.length() > 25) {
...
//                }
            ...
            // Set audit fields
            requestBean.setString("updated_by", currentUser);
            requestBean.setString("action_id", actionId);

            LOG.info("Processing timetable update request for channel: {}, entries: {}, user: {}",
                    channelCode, switchDetailsArray.size(), currentUser);

            // Update records in database
            boolean success = TimetableDbHelper.updateTimetableRecords(requestBean);

            if (success) {
                LOG.info("Timetable update completed successfully for channel: {} by user: {}",
                        channelCode, currentUser);

                // Create audit log for successful update
                MessageDbHelper.createMessageHelper(
                        "Timetable Update Completed",
                        "Timetable records updated for channel: " + channelCode + " with " + switchDetailsArray.size() + " entries",
                        currentUser,
                        AppModules.TRANSACTION_MANAGEMENT,
                        currentUser
                );

                return createSuccessResponse("00", "Timetable record updated successfully");
            } else {
                String message = requestBean.getString("message") != null ?
                        requestBean.getString("message") : "Failed to update timetable records";

                // Create audit log for failed update
                MessageDbHelper.createMessageHelper(
                        "Timetable Update Failed",
                        "Failed to update timetable records for channel: " + channelCode + " - " + message,
                        currentUser,
                        AppModules.TRANSACTION_MANAGEMENT,
                        currentUser
                );
            ...
            LOG.error("Error processing timetable update request for channel: {}, user: {}",
                    requestBean.getString("channel_code"), currentUser, e);

            // Create audit log for exception
            MessageDbHelper.createMessageHelper(
                    "Timetable Update Error",
                    "Error occurred during timetable update: " + e.getMessage(),
                    currentUser,
                    AppModules.TRANSACTION_MANAGEMENT,
                    currentUser
            );
